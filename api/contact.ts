...
type ContactRequestBody = z.infer<typeof contactFormSchema>;
...
export default async function handler(req: any, res: any) {
  ...
  if (!mailUser || !mailPass || !mailFrom) {
    return res.status(500).json({ message: "Falta configuración de correo en el servidor" });
  }
  ...
  const { nombre, email, telefono, tipoProyecto, mensaje } = parsed.data as ContactRequestBody;
